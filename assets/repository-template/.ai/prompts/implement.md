...
5. 可以运行用于实现反馈的目标检查，但不要替代独立 TEST 阶段；
6. 把变更摘要、实际修改、配置符合性和计划偏差写入 result.md；
7. 实现完成后将 status 更新为 TESTING，停止实现并进入 TEST 阶段；
8. 不得在 IMPLEMENT 阶段把任务标记为 VERIFIED 或 DONE。
