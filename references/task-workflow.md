...
Complete `task.md` from the user's request and repository evidence. Do not invent acceptance criteria that materially change scope. Classify risk using `.ai/conventions.md`.

## Plan

Read the required project context and the new task. Use `.ai/prompts/plan.md` as stage guidance. Perform read-only investigation and complete `plan.md` with concrete files/modules, impacts, tests, risks, and rollback.
...
Use `.ai/prompts/implement.md`. Set status to `IMPLEMENTING`, stay within the approved plan, and preserve unrelated changes. If scope or risk changes materially, update the plan and renew approval when required.
...
Record exact commands, environment, outcomes, skipped checks, alternative evidence, failures, reruns, and residual risk in `test.md`. If a failure requires code changes, return the task to `IMPLEMENTING`, fix it, and rerun the affected and regression checks. Set `test_status: passed` and status `VERIFIED` only after all required tests pass.

## Review

Use `.ai/prompts/review.md`. Review the actual diff and `test.md` against the task and plan, prioritizing correctness, boundary cases, error recovery, concurrency and idempotency, security, compatibility, data changes, performance, observability, test sufficiency, and documentation consistency. Do not approve when `test_status` is not `passed`.
